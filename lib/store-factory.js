...
var getter = require('./utils.js').getter;
...
module.exports = function StateStoreFactory(Immutable, EventEmitter, _) {
  ...
  function StateStore(initialState) {
    ...
    var history, historyIndex, emitter, pendingStateUpdates, reduceState, queueReduce;
    ...
    */
    reduceState = function reduceState(currentImmutableState) {
      var relevantReducers = StateStore.getRelevantReducers(reducers);

      var maybeNewState = relevantReducers.reduce(function (lastImmutableState, reducer) {
        var newState = void 0;

        if (reducer.type === Reducer) {
          var delta = reducer.delta;
        ...
        if (!_.isPlainObject(newState)) throw new Error(errors.INVALID_RETURN);
    ...
    */
    queueReduce = function queueReduce(index, newState) {
      ...
      pendingDeltas = _.filter(reducers, function (reducer) {
        reducer.delta !== undefined;
      });
      // defer a state reduction on the next tick if one isn't already queued
      if (pendingDeltas.length === 1) setTimeout(function () {
        return reduceState(_this.getImmutableState());
      }, 0);
    }.bind(this);
    ...
    */
    this.addReducer = function addReducer(reducer) {
      if (reducer.$$factory !== Reducer || reducer.$$factory !== Hook) throw new TypeError(errors.INVALID_REDUCER);
      ...
      var reducerIndex = reducers.length - 1;

      // kick off a reduce cycle when the reducer action is called anywhere in the app
      reducer.$$bind(function (newState) {
        return queueReduce(reducerIndex, newState);
      });
      this.emitter.emit('REDUCER_ADDED', reducer);
    }.bind(this);

    // set the store's first reducer to handle direct setState operations
    var stateSetter = new Reducer(function (lastState, delta) {
      return _.merge.apply(_, [{}].concat(delta.pendingSetOps));
    });
    ...
    */
    this.getStateAtVersion = function getStateAtVersion(index) {
      if (history[index]) return history[index].toJS();
    };
  ...
  StateStore.prototype = {
    ...
    */
    ...
    addListener: function addListener(callback, thisBinding) {
  ...
  */
  StateStore.getRelevantReducers = function (reducersArray) {
    index = _.reduce(reducersArray, function (res, reducer, i) {
      if (reducer.delta !== undefined) res.push(i);
      return res;
    }, []).sort(function (a, b) {
      return a - b;
    })[0];
